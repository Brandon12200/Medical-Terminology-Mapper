...
medical terms to standardized terminologies when exact matches are not found.
...
# Try to import advanced NLP libraries, but provide fallbacks if not available
try:
...
class FuzzyMatcher:
    ...
    This class provides methods for fuzzy matching medical terms to
    ...
    def __init__(self, db_manager, config: Optional[Dict[str, Any]] = None):
        ...
        # In-memory term index for faster matching
        self.term_index = {
            "snomed": {},
            "loinc": {},
            "rxnorm": {}
        ...
        # Common word replacements for medical terms
    ...
    def _load_stopwords(self) -> List[str]:
        """
        Load stopwords to ignore during matching.
    ...
    def _build_index(self, system: str) -> bool:
        ...
        try:
            ...
            for code, term, display in rows:
                ...
                # Add to term lists for vectorization
    ...
    def _find_cosine_match(self, term: str, system: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Dictionary with mapping information or None if no good match
        """
        if not HAS_SKLEARN or system not in self.vector_matrices or not self.vectorizer:
            return None

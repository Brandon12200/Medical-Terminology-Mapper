    ...
    def lookup_snomed(self, term: str) -> Optional[Dict[str, Any]]:
        ...
        if "snomed" in self.connections:
            try:
                ...
                cursor.execute(
                    "SELECT code, display FROM snomed_concepts WHERE LOWER(term) LIKE ? AND is_active = 1 LIMIT 1", 
                    (f"%{term.lower()}%",)
                )
                result = cursor.fetchone()
    ...
    def lookup_loinc(self, term: str) -> Optional[Dict[str, Any]]:
        ...
        if "loinc" in self.connections:
            try:
                ...
                cursor.execute(
                    "SELECT code, display FROM loinc_concepts WHERE component LIKE ? LIMIT 1", 
                    (f"%{term}%",)
                )
                result = cursor.fetchone()
                ...
                if result:
                    return {
                        ...
                        "system": "http://loinc.org",
                        "found": True,
                        "confidence": 0.7  # Lower confidence for component match
                    }
    ...
    def lookup_rxnorm(self, term: str) -> Optional[Dict[str, Any]]:
        ...
        if "rxnorm" in self.connections:
            try:
                ...
                # Try partial match for medication names
                cursor.execute(
                    "SELECT code, display FROM rxnorm_concepts WHERE LOWER(term) LIKE ? AND is_active = 1 LIMIT 1", 
                    (f"%{term.lower()}%",)
                )
                result = cursor.fetchone()
